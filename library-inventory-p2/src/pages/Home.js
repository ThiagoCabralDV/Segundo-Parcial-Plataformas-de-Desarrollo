...
export default function Home() {
  ...
  const removeBook = (title) => setBooks(books.filter(b => b.title !== title));

  const startEdit = (book) => setEditingBook(book);

  const editBook = (updatedBook) => {
    setBooks(books.map(b => b.title === editingBook.title ? updatedBook : b));
    setEditingBook(null);
  };
  ...
  return (
    <div className="container mt-4">
      ...
      <BookTable books={books} removeBook={removeBook} startEdit={startEdit} />
    </div>
  );
}
